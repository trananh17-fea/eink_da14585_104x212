...
struct gapm_start_advertise_cmd* my_app_easy_gap_undirected_advertise_start_create_msg(void)
{
    ...
    if (adv_cmd == NULL)
    {
        ...
        if (USER_DEVICE_NAME_LEN > 0)
        {
            ...
            // Get remaining space in the Scan Response Data ( plus 2 bytes are used for the length and flag bytes of the Device Name)
            uint16_t total_scan_space = adv_cmd->info.host.scan_rsp_data_len + 2 + USER_DEVICE_NAME_LEN;
            uint8_t device_name[USER_DEVICE_NAME_LEN+1]={0};
            uint8_t device_mac[6]={0};
            // ble_get_mac_address(device_mac,6);
            nvds_tag_len_t maclen = 0;
            nvds_get_func(NVDS_TAG_BD_ADDRESS,&maclen,device_mac);
            memcpy(device_name,USER_DEVICE_NAME,USER_DEVICE_NAME_LEN);
            sprintf((char *)&device_name[9],"%02X%02X%02X",device_mac[2],device_mac[1],device_mac[0]);
